...
#
# FINAL UPDATE:
# 1) Support fault profiles:
#    - noncritical
#    - critical-proxy
#    - custom
# 2) Exclude orderer.example.com from noncritical by default
# 3) Strong chaincode deploy verification after every reset
# 4) Fix false-negative lifecycle verification
# 5) Keep retrieve semantics correct:
#    - reset network
#    - preload submit without fault on fresh ledger
#    - run retrieve with runtime fault injection on same fresh ledger
# 6) Marker-based runtime injection retained
# 7) Caliper docker monitor disabled by default for fault runs
# 8) External docker stats sampler used for resource CSV
# 9) Remove duplicate resource parser function
# 10) Resolve and log actual fault set only after network is ready
# ==========================================================
...
K_FAULT="${4:-3}"                  # number of orderers to crash
TOTAL_REPETITIONS="${5:-5}"        # repetitions
MODE="${6:-unsplit}"               # split | unsplit
FAULT_PROFILE="${7:-noncritical}"  # noncritical | critical-proxy | custom
...
# ----------------------------------------------------------
# Fault injection / selection knobs
# ----------------------------------------------------------
FAULT_CMD="${FAULT_CMD:-stop0}"                         # stop0 | stop | kill
SELECTION_POLICY="${SELECTION_POLICY:-fixed-highest}"   # fixed-highest | fixed-lowest | rotate
FAULT_SET_OVERRIDE="${FAULT_SET_OVERRIDE:-}"            # comma-separated exact names for custom profile
ORDERER_REGEX="${ORDERER_REGEX:-^orderer([0-9]+)?\.example\.com$}"
EXPECTED_ORDERERS="${EXPECTED_ORDERERS:-10}"

# Fault profile semantics
CRITICAL_ROOT_ORDERER="${CRITICAL_ROOT_ORDERER:-orderer.example.com}"
NONCRITICAL_EXCLUDE_CSV="${NONCRITICAL_EXCLUDE_CSV:-orderer.example.com}"
...
# override timeouts inside generated YAML
...
PHASE_RESOURCE_PID=""
CURRENT_REP_FOR_ROTATE=1

# ----------------------------------------------------------
# CSV headers
# ----------------------------------------------------------
if [ ! -s "$SUMMARY_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,selection_policy,fault_nodes,round_label,name,succ,fail,send_rate_tps,max_latency_s,min_latency_s,avg_latency_s,throughput_tps" > "$SUMMARY_CSV"
fi

if [ ! -s "$RESOURCE_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,selection_policy,fault_nodes,round_label,container,cpu_avg,cpu_max,mem_avg_mb,mem_max_mb,traffic_in_mb,traffic_out_mb,disc_read_b,disc_write_mb" > "$RESOURCE_CSV"
fi

if [ ! -s "$FAULT_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,selection_policy,fault_nodes,fault_cmd,inject_epoch_s,inject_iso" > "$FAULT_CSV"
fi
...
# Fault-set logic
# ----------------------------------------------------------
list_orderers_excluding_csv() {
  local excluded_csv="${1:-}"
  local excluded=",$(echo "$excluded_csv" | tr '|' ','),"
...
pick_from_array() {
  ...
  [ "$k" -le "$n" ] || {
    echo "ERROR: requested k=${k} but only ${n} candidates available" >&2
    return 1
  }
  ...
  case "$mode" in
    ...
    fixed-lowest)
      ...
      ;;
    rotate)
      local start=$(( (CURRENT_REP_FOR_ROTATE - 1) % n ))
      for (( i=0; i<k; i++ )); do
        chosen+=("${arr[$(( (start + i) % n ))]}")
      done
      ;;
    *)
      echo "ERROR: unsupported SELECTION_POLICY=${mode}" >&2
      return 1
      ;;
  esac
...
calc_fault_set() {
  ...
  case "$FAULT_PROFILE" in
    custom)
      if [ -z "$FAULT_SET_OVERRIDE" ]; then
        echo "ERROR: FAULT_PROFILE=custom requires FAULT_SET_OVERRIDE" >&2
        return 1
      fi
      echo "$FAULT_SET_OVERRIDE" | tr ',' '\n' | sed '/^$/d'
      return 0
      ;;

    noncritical)
      mapfile -t candidates < <(list_orderers_excluding_csv "$NONCRITICAL_EXCLUDE_CSV")
      [ "${#candidates[@]}" -gt 0 ] || {
        echo "ERROR: no candidates for FAULT_PROFILE=noncritical" >&2
        return 1
      }
      pick_from_array "$SELECTION_POLICY" "$k" "${candidates[@]}"
      ;;

    critical-proxy)
      local chosen=()

      if ! list_real_orderers | grep -Fxq "$CRITICAL_ROOT_ORDERER"; then
        echo "ERROR: CRITICAL_ROOT_ORDERER not found: ${CRITICAL_ROOT_ORDERER}" >&2
        return 1
      fi

      chosen+=("$CRITICAL_ROOT_ORDERER")

      mapfile -t rest < <(list_orderers_excluding_csv "$CRITICAL_ROOT_ORDERER")
      [ "${#rest[@]}" -gt 0 ] || {
        echo "ERROR: no remaining orderers for FAULT_PROFILE=critical-proxy" >&2
        return 1
      }

      local need_more=$(( k - ${#chosen[@]} ))
      if [ "$need_more" -gt 0 ]; then
        local i
        for (( i=0; i<${#rest[@]} && ${#chosen[@]}<k; i++ )); do
          chosen+=("${rest[$i]}")
        done
      fi

      [ "${#chosen[@]}" -ge "$k" ] || {
        echo "ERROR: not enough nodes for FAULT_PROFILE=critical-proxy, chosen=${#chosen[@]}, need=${k}" >&2
        return 1
      }
    ...
    *)
      echo "ERROR: unsupported FAULT_PROFILE=${FAULT_PROFILE}" >&2
      return 1
      ;;
  esac
}
...
inject_faults_now() {
  ...
  log "Injecting faults: rep=${rep} scope=${scope} phase=${phase} profile=${FAULT_PROFILE} k=${k} nodes=${ACTUAL_FAULT_NODES_CSV} cmd=${FAULT_CMD}"
  ...
  epoch="$(date +%s)"
  iso="$(date -Iseconds)"
  echo "${SCENARIO_ID},${rep},${scope},${phase},${MODE},${CONSENSUS},${BATCH},${TPS},${k},${SELECTION_POLICY},${ACTUAL_FAULT_NODES_CSV},${FAULT_CMD},${epoch},${iso}" >> "$FAULT_CSV"
}
...
append_resource_rows_from_external_sampler() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$scope" "$phase" "$MODE" "$CONSENSUS" "$BATCH" "$TPS" "$K_FAULT" "$SELECTION_POLICY" "$fault_nodes" "$raw_csv" "$target_round" >> "$RESOURCE_CSV" <<'PY'
import csv, sys
from collections import defaultdict

scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, sel, fault_nodes, raw_csv, target = sys.argv[1:]
...
for container, arr in sorted(by_container.items()):
    ...
    print(",".join([
        scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, sel, fault_nodes,
        target, container,
...
append_summary_row() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$scope" "$phase" "$MODE" "$CONSENSUS" "$BATCH" "$TPS" "$K_FAULT" "$SELECTION_POLICY" "$fault_nodes" "$html" "$target_round" >> "$SUMMARY_CSV" <<'PY'
import sys, re
from html import unescape

scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, sel, fault_nodes, path, target = sys.argv[1:]
...
if not m:
    print(f"{scenario},{rep},{scope},{phase},{mode},{cons},{batch},{tps},{k_fault},{sel},{fault_nodes},{target},ERROR_NO_BENCHMARKSUMMARY,0,0,,,,")
    raise SystemExit(0)
...
if not row_dict:
    print(f"{scenario},{rep},{scope},{phase},{mode},{cons},{batch},{tps},{k_fault},{sel},{fault_nodes},{target},ERROR_TARGET_NOT_FOUND,0,0,,,,")
    raise SystemExit(0)
...
run_phase_with_runtime_injection() {
  ...
  if wait_for_round_start_marker "$phase_log" "$marker" "$ROUND_MARKER_TIMEOUT_SEC"; then
    ...
    if kill -0 "$caliper_pid" >/dev/null 2>&1; then
      inject_faults_now "$rep" "$scope" "$phase" "$K_FAULT"
    else
...
FAULT_SET_OVERRIDE=${FAULT_SET_OVERRIDE}
CRITICAL_ROOT_ORDERER=${CRITICAL_ROOT_ORDERER}
NONCRITICAL_EXCLUDE_CSV=${NONCRITICAL_EXCLUDE_CSV}
...
for (( rep=1; rep<=TOTAL_REPETITIONS; rep++ )); do
  ...
  log "Fault profile: ${FAULT_PROFILE}"
  log "##########################################################"
