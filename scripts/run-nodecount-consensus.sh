...
# ==========================================================
# NODE COUNT EXPERIMENT (4 / 7 / 10 ORDERERS)
# 5 REPS, RESET NETWORK EACH REP (ONE SCENARIO) + ANALYTICS CSV
#
# TUJUAN:
# - Mengukur dampak jumlah orderer: 4, 7, 10
# - Konsensus: Raft / SmartBFT
# - Batch default: 100
# - TPS default: 100
#
# POLA:
# - Reset network setiap repetisi
# - Deploy chaincode ulang setiap repetisi
# - Jalankan 2 phase: SUBMIT lalu RETRIEVE
# - Ekstrak summary metrics + resource metrics dari report.html
#
# CATATAN PENTING:
# - Topologi 4 / 7 / 10 orderer diasumsikan SUDAH Anda siapkan manual
#   di environment/config yang aktif saat script dijalankan.
# - Script ini tidak mengubah compose/configtx jumlah orderer.
# - ORDERER_COUNT dipakai untuk VALIDASI bahwa topologi aktif sesuai
#   dengan skenario yang sedang diuji.
# ==========================================================

CONSENSUS="${1:-SmartBFT}"      # "Raft" / "SmartBFT"
ORDERER_COUNT="${2:-4}"         # 4 / 7 / 10
BATCH="${3:-100}"               # default 100
TPS="${4:-100}"                 # default 100
TOTAL_REPETITIONS="${5:-5}"     # default 5
...
POST_DEPLOY_WAIT_SEC="${POST_DEPLOY_WAIT_SEC:-10}"

# --------------------------
# Optional strict validation
# --------------------------
STRICT_ORDERER_COUNT_CHECK="${STRICT_ORDERER_COUNT_CHECK:-1}"
...
SCENARIO_ID="${CONSENSUS}_n${ORDERER_COUNT}_b${BATCH}_tps${TPS}"
RESULT_DIR="research-results/${SCENARIO_ID}"
mkdir -p "$RESULT_DIR"

SUMMARY_CSV="${RESULT_DIR}/${CONSENSUS}_n${ORDERER_COUNT}_b${BATCH}_tps${TPS}_summary_metrics.csv"
RESOURCE_CSV="${RESULT_DIR}/${CONSENSUS}_n${ORDERER_COUNT}_b${BATCH}_tps${TPS}_resource_metrics.csv"
EXEC_LOG="${RESULT_DIR}/${SCENARIO_ID}_execution_log.txt"
touch "$EXEC_LOG"

# ========= CSV Headers =========
if [ ! -s "$SUMMARY_CSV" ]; then
  echo "scenario,rep,phase,consensus,orderer_count,batch,tps,round_label,name,succ,fail,send_rate_tps,max_latency_s,min_latency_s,avg_latency_s,throughput_tps" > "$SUMMARY_CSV"
fi

if [ ! -s "$RESOURCE_CSV" ]; then
  echo "scenario,rep,phase,consensus,orderer_count,batch,tps,round_label,container,cpu_avg,cpu_max,mem_avg_mb,mem_max_mb,traffic_in_mb,traffic_out_mb,disc_read_b,disc_write_mb" > "$RESOURCE_CSV"
fi
...
validate_inputs() {
  case "$CONSENSUS" in
    Raft|SmartBFT) ;;
    *)
      echo "ERROR: CONSENSUS harus 'Raft' atau 'SmartBFT', dapat: $CONSENSUS" >&2
      exit 1
      ;;
  esac
  ...
  case "$ORDERER_COUNT" in
    4|7|10) ;;
    *)
      echo "ERROR: ORDERER_COUNT harus salah satu dari 4 / 7 / 10, dapat: $ORDERER_COUNT" >&2
      exit 1
      ;;
  esac
}
...
pick_sk_file_latest() {
  ...
  if [ -z "$sk" ]; then
    sk="$(ls -t "${keystore_dir}"/priv_sk 2>/dev/null | head -n 1 || true)"
  fi
  if [ -z "$sk" ]; then
    sk="$(ls -t "${keystore_dir}"/*.pem 2>/dev/null | head -n 1 || true)"
  fi

  if [ -z "$sk" ] || [ ! -f "$sk" ]; then
    echo "ERROR: tidak menemukan private key di ${keystore_dir}" >&2
    return 1
  fi
  echo "$sk"
}
...
get_running_orderer_count() {
  docker ps --format '{{.Names}}' 2>/dev/null | awk '
    BEGIN { IGNORECASE=1; c=0 }
    /orderer/ && !/ca/ && !/couchdb/ && !/peer/ && !/cli/ { c++ }
    END { print c+0 }
  '
}

assert_expected_orderer_count() {
  local actual
  actual="$(get_running_orderer_count || echo 0)"

  if [ "$STRICT_ORDERER_COUNT_CHECK" = "1" ]; then
    if [ "$actual" -ne "$ORDERER_COUNT" ]; then
      echo "ERROR: jumlah orderer aktif = ${actual}, tetapi skenario meminta ${ORDERER_COUNT}" >&2
      echo "Pastikan topologi ${ORDERER_COUNT} orderer sudah aktif/terkonfigurasi benar." >&2
      exit 1
    fi
  else
    echo "INFO: STRICT_ORDERER_COUNT_CHECK=0, current running orderers=${actual}, expected=${ORDERER_COUNT}"
  fi
}

# ==========================================================
# RESET NETWORK SETIAP REPETISI
# (mengikuti pola batch/TPS sebelumnya)
# ==========================================================
reset_network() {
  echo "🧱 Reset network (${CONSENSUS}, target orderers=${ORDERER_COUNT})..."
  ...
  # 4) Start fresh
  #    Topologi 4/7/10 mengikuti config manual yang sedang aktif.
  ...
  echo "⏳ Waiting ${POST_DEPLOY_WAIT_SEC}s after deployCC..."
...
# - receipts path/dir
# - TPS untuk round REAL
# - retrieve readOnly=false
# - force funcRetrieve=retrieveEvidenceAndLog jika field ada
# ==========================================================
patch_bench_yaml() {
...
append_summary_row() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$phase" "$CONSENSUS" "$ORDERER_COUNT" "$BATCH" "$TPS" "$html" "$target_round" >> "$SUMMARY_CSV" <<'PY'
import sys, re
from html import unescape

scenario, rep, phase, cons, orderer_count, batch, tps, path, target = sys.argv[1:]
...
if not m:
    print(f"{scenario},{rep},{phase},{cons},{orderer_count},{batch},{tps},{target},ERROR_NO_BENCHMARKSUMMARY,0,0,,,,")
    raise SystemExit(0)
...
if len(parsed) < 2:
    print(f"{scenario},{rep},{phase},{cons},{orderer_count},{batch},{tps},{target},ERROR_EMPTY_SUMMARY,0,0,,,,")
    raise SystemExit(0)
...
if not row_dict:
    print(f"{scenario},{rep},{phase},{cons},{orderer_count},{batch},{tps},{target},ERROR_TARGET_NOT_FOUND,0,0,,,,")
    raise SystemExit(0)

def g(k): return row_dict.get(k, "")

print(",".join([
    scenario, rep, phase, cons, orderer_count, batch, tps, target,
    g("Name") or target,
    g("Succ") or "0",
    g("Fail") or "0",
    g("Send Rate (TPS)"),
...
# - tetap satu CSV seperti baseline
# - berisi per-container + ORDERERS_AGG
# ==========================================================
append_resource_rows() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$phase" "$CONSENSUS" "$ORDERER_COUNT" "$BATCH" "$TPS" "$html" "$target_round" >> "$RESOURCE_CSV" <<'PY'
import sys, re
from html import unescape
...
for r in parsed[header_idx+1:]:
    ...
    lname = name.lower()
    if ("orderer" in lname) and ("ca" not in lname) and ("peer" not in lname) and ("couchdb" not in lname) and ("cli" not in lname):
...
echo "ORDERER_COUNT : $ORDERER_COUNT"
echo "BATCH         : $BATCH"
echo "TPS           : $TPS"
echo "REPETITIONS   : $TOTAL_REPETITIONS"
