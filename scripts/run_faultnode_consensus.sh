...
# ==========================================================
# Final fault-node experiment runner for Hyperledger Fabric v3
# Raft vs SmartBFT, runtime orderer crash injection
#
# FINAL HARDENED PAPER VERSION:
# 1) Fixed fault placements only:
#    - limited-impact : {orderer8, orderer9, orderer10}
#    - high-impact    : {orderer,  orderer9, orderer10}
# 2) K_FAULT must be 3
# 3) Strong chaincode deploy verification after every reset
# 4) Retrieve semantics preserved:
#    - reset network
#    - preload submit without fault on fresh ledger
#    - run retrieve with runtime fault injection on same fresh ledger
# 5) Marker-based runtime injection retained
# 6) Caliper docker monitor disabled by default for fault runs
# 7) External docker stats sampler used for resource CSV
# 8) Fault profile normalization + alias support
# 9) Runtime fault injection failure invalidates the run
# ==========================================================
...
BATCH="${2:-100}"                    # label only
TPS="${3:-100}"                      # offered TPS
K_FAULT="${4:-3}"                    # fixed to 3 for final experiment
TOTAL_REPETITIONS="${5:-5}"          # repetitions
MODE="${6:-unsplit}"                 # split | unsplit
FAULT_PROFILE_RAW="${7:-limited-impact}"  # limited-impact | high-impact
...
POSTFAULT_GAP_SEC="${POSTFAULT_GAP_SEC:-2}"
...
# Fixed fault placements agreed for paper
LIMITED_IMPACT_SET="${LIMITED_IMPACT_SET:-orderer8.example.com,orderer9.example.com,orderer10.example.com}"
HIGH_IMPACT_SET="${HIGH_IMPACT_SET:-orderer.example.com,orderer4.example.com,orderer5.example.com}"
...
PHASE_RESOURCE_PID=""
FAULT_PROFILE=""
FAULT_PROFILE_ORIG="$FAULT_PROFILE_RAW"
...
normalize_fault_profile() {
  ...
  case "$lc" in
    limited-impact|limited_impact|limitedimpact|limited)
      printf '%s\n' "limited-impact"
      ;;
    high-impact|high_impact|highimpact|high)
      printf '%s\n' "high-impact"
      ;;
    *)
      printf '%s\n' "$raw"
...
validate_fault_profile() {
  case "$FAULT_PROFILE" in
    limited-impact|high-impact)
      ...
      ;;
    *)
      echo "ERROR: unsupported FAULT_PROFILE='${FAULT_PROFILE_ORIG}' (normalized='${FAULT_PROFILE}'). Use: limited-impact | high-impact" >&2
      return 1
      ;;
  esac
}
...
# ----------------------------------------------------------
# Normalize profile before outputs are derived
# ----------------------------------------------------------
FAULT_PROFILE="$(normalize_fault_profile "$FAULT_PROFILE_RAW")"

# ----------------------------------------------------------
# Outputs
# ----------------------------------------------------------
SCENARIO_ID="${CONSENSUS}_b${BATCH}_tps${TPS}_k${K_FAULT}_${MODE}_${FAULT_PROFILE}"
...
if [ ! -s "$SUMMARY_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,fault_profile,fault_nodes,round_label,name,succ,fail,send_rate_tps,max_latency_s,min_latency_s,avg_latency_s,throughput_tps" > "$SUMMARY_CSV"
fi

if [ ! -s "$RESOURCE_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,fault_profile,fault_nodes,round_label,container,cpu_avg,cpu_max,mem_avg_mb,mem_max_mb,traffic_in_mb,traffic_out_mb,disc_read_b,disc_write_mb" > "$RESOURCE_CSV"
fi

if [ ! -s "$FAULT_CSV" ]; then
  echo "scenario,rep,scope,phase,mode,consensus,batch,tps,k_fault,fault_profile,fault_nodes,fault_cmd,inject_epoch_s,inject_iso" > "$FAULT_CSV"
fi
...
get_fault_profile_csv() {
  case "$FAULT_PROFILE" in
    ...
    high-impact)
      ...
      ;;
    *)
      echo "ERROR: unsupported FAULT_PROFILE='${FAULT_PROFILE_ORIG}' (normalized='${FAULT_PROFILE}')" >&2
      return 1
      ;;
  esac
}

verify_fault_targets_exist_in_running_orderers() {
  local nodes=("$@")
  local missing=0
...
calc_fault_set() {
  ...
  if [ "$k" -ne 3 ]; then
    echo "ERROR: fixed fault-placement experiment requires K_FAULT=3, but got k=${k}" >&2
    return 1
  fi

  chosen_csv="$(get_fault_profile_csv)" || return 1

  while IFS= read -r node; do
    node="$(trim_string "$node")"
    [ -z "$node" ] && continue
    nodes+=("$node")
  done < <(echo "$chosen_csv" | tr ',' '\n')

  [ "${#nodes[@]}" -eq 3 ] || {
    echo "ERROR: resolved fault-set size is ${#nodes[@]}, expected 3" >&2
    return 1
  }

  verify_fault_targets_exist_in_running_orderers "${nodes[@]}" || return 1
...
format_fault_nodes_csv() {
  ...
  local k="$2"
  local nodes
  nodes="$(calc_fault_set "$rep" "$k" | paste -sd'|' -)"
  echo "$nodes"
}
...
inject_faults_now() {
  ...
  mapfile -t nodes < <(calc_fault_set "$rep" "$k")
  [ "${#nodes[@]}" -gt 0 ] || { echo "ERROR: empty fault-set" >&2; return 1; }

  ACTUAL_FAULT_NODES_CSV="$(printf '%s|' "${nodes[@]}" | sed 's/|$//')"
  log "Injecting fixed fault placement: rep=${rep} scope=${scope} phase=${phase} profile=${FAULT_PROFILE} k=${k} nodes=${ACTUAL_FAULT_NODES_CSV} cmd=${FAULT_CMD}"
  ...
  epoch="$(date +%s)"
  iso="$(date -Iseconds)"
  echo "${SCENARIO_ID},${rep},${scope},${phase},${MODE},${CONSENSUS},${BATCH},${TPS},${k},${FAULT_PROFILE},${ACTUAL_FAULT_NODES_CSV},${FAULT_CMD},${epoch},${iso}" >> "$FAULT_CSV"
}
...
append_resource_rows_from_external_sampler() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$scope" "$phase" "$MODE" "$CONSENSUS" "$BATCH" "$TPS" "$K_FAULT" "$FAULT_PROFILE" "$fault_nodes" "$raw_csv" "$target_round" >> "$RESOURCE_CSV" <<'PY'
import csv, sys
from collections import defaultdict

scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, profile, fault_nodes, raw_csv, target = sys.argv[1:]
...
for container, arr in sorted(by_container.items()):
    ...
    print(",".join([
        scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, profile, fault_nodes,
        target, container,
...
build_submit_bench() {
  ...
  python3 - "$src" "$dst" "$variant" "$tps_real" "$receipts_real" "$receipts_warmup" "$interval" \
    "$WARMUP_SEC" "$WARMUP_TPS" "$UNSPLIT_REAL_SEC" "$PREFAULT_SEC" "$POSTFAULT_SEC" \
...
if variant == 'unsplit':
    ...
    new_rounds = [
        set_submit_round(warm, 'submit-warmup', warmup_sec, warmup_tps, receipts_warmup, submit_warmup_seed),
        set_submit_round(rl, f'{submit_real_label}-preFault', prefault_sec, tps_real, receipts_real, submit_real_seed),
    ]
elif variant == 'post':
    new_rounds = [
        set_submit_round(rl, f'{submit_real_label}-postFault', postfault_sec, tps_real, receipts_real, submit_real_seed),
    ]
...
build_retrieve_bench() {
  local src="$1" dst="$2" variant="$3" tps_real="$4" receipts_path="$5" interval="$6"

  python3 - "$src" "$dst" "$variant" "$tps_real" "$receipts_path" "$interval" \
    "$WARMUP_SEC" "$WARMUP_TPS" "$UNSPLIT_REAL_SEC" "$PREFAULT_SEC" "$POSTFAULT_SEC" \
    "$RETRIEVE_REAL_LABEL" "$WORKERS" "$RETRIEVE_TIMEOUT_SEC" "$ENABLE_CALIPER_RESOURCE_MONITOR" <<'PY'
import sys, copy, yaml

(src, dst, variant, tps_real, receipts_path, interval,
 warmup_sec, warmup_tps, unsplit_real_sec, prefault_sec, postfault_sec,
...
if variant == 'unsplit':
    ...
    new_rounds = [
        set_retrieve_round(warm, 'retrieve-warmup', warmup_sec, warmup_tps, receipts_path),
        set_retrieve_round(rl, f'{retrieve_real_label}-preFault', prefault_sec, tps_real, receipts_path),
    ]
elif variant == 'post':
    new_rounds = [
        set_retrieve_round(rl, f'{retrieve_real_label}-postFault', postfault_sec, tps_real, receipts_path),
    ]
else:
    raise ValueError(f'Unsupported retrieve variant: {variant}')
...
append_summary_row() {
  ...
  python3 - "$SCENARIO_ID" "$rep" "$scope" "$phase" "$MODE" "$CONSENSUS" "$BATCH" "$TPS" "$K_FAULT" "$FAULT_PROFILE" "$fault_nodes" "$html" "$target_round" >> "$SUMMARY_CSV" <<'PY'
import sys, re
from html import unescape

scenario, rep, scope, phase, mode, cons, batch, tps, k_fault, profile, fault_nodes, path, target = sys.argv[1:]
...
if not m:
    print(f"{scenario},{rep},{scope},{phase},{mode},{cons},{batch},{tps},{k_fault},{profile},{fault_nodes},{target},ERROR_NO_BENCHMARKSUMMARY,0,0,,,,")
    raise SystemExit(0)
...
if not row_dict:
    print(f"{scenario},{rep},{scope},{phase},{mode},{cons},{batch},{tps},{k_fault},{profile},{fault_nodes},{target},ERROR_TARGET_NOT_FOUND,0,0,,,,")
    raise SystemExit(0)
...
run_phase() {
  ...
  log "Launching Caliper: rep=${rep} scope=${scope} phase=${phase} round=${target_round}"
  ...
  if [ -f report.html ]; then
    append_summary_row "$rep" "$scope" "$phase" "report.html" "$target_round" "$fault_nodes"
    [ -f "$PHASE_RESOURCE_RAW" ] && append_resource_rows_from_external_sampler "$rep" "$scope" "$phase" "$PHASE_RESOURCE_RAW" "$target_round" "$fault_nodes"
    mv report.html "${RESULT_DIR}/${prefix}.html"
    [ -f report.json ] && mv report.json "${RESULT_DIR}/${prefix}.json" || true
    if [ "$rc" -ne 0 ]; then
      log "Caliper exited rc=${rc} but produced report: rep=${rep} scope=${scope} phase=${phase}"
    fi
    log "Caliper OK: rep=${rep} scope=${scope} phase=${phase}"
    return 0
  fi

  log "Caliper FAIL(no report): rep=${rep} scope=${scope} phase=${phase}"
  return 1
}
...
run_phase_with_runtime_injection() {
  ...
  log "Launching Caliper with runtime marker-based injector: rep=${rep} scope=${scope} phase=${phase} planned_fault_nodes=${fault_nodes}"
  ...
  if wait_for_round_start_marker "$phase_log" "$marker" "$ROUND_MARKER_TIMEOUT_SEC"; then
    ...
    if kill -0 "$caliper_pid" >/dev/null 2>&1; then
      if inject_faults_now "$rep" "$scope" "$phase" "$K_FAULT"; then
        injection_ok=true
      else
        log "ERROR: fault injection failed for rep=${rep} scope=${scope} phase=${phase}"
        kill "$caliper_pid" >/dev/null 2>&1 || true
      fi
    else
      log "WARNING: Caliper exited before fault injection for rep=${rep} scope=${scope} phase=${phase}"
    fi
  else
    log "ERROR: Failed to detect real-round marker within timeout: ${marker}"
    kill "$caliper_pid" >/dev/null 2>&1 || true
  fi
  ...
  if [ "$injection_ok" != "true" ]; then
    log "INVALID RUN: runtime fault injection did not succeed: rep=${rep} scope=${scope} phase=${phase}"
    return 1
  fi
  ...
  if [ -f report.html ]; then
    ...
    mv report.html "${RESULT_DIR}/${prefix}.html"
    [ -f report.json ] && mv report.json "${RESULT_DIR}/${prefix}.json" || true
    log "Caliper OK(with runtime marker injector): rep=${rep} scope=${scope} phase=${phase}"
    return 0
  fi

  log "Caliper FAIL(no report, with runtime marker injector): rep=${rep} scope=${scope} phase=${phase}"
  return 1
}
...
run_submit_experiment() {
  ...
  local submit_pre_dir="${submit_root}/receipts_pre"
  local submit_post_dir="${submit_root}/receipts_post"
  local submit_unsplit_dir="${submit_root}/receipts_unsplit"
  mkdir -p "$submit_pre_dir" "$submit_post_dir" "$submit_unsplit_dir"
  ...
  local fault_nodes
  fault_nodes="$(format_fault_nodes_csv "$rep" "$K_FAULT")"
  log "Submit fault set (resolved after network ready): ${fault_nodes}"

  if [ "$MODE" = "split" ]; then
    local bench_pre="${RESULT_DIR}/patched_submit_rep${rep}_pre.yaml"
    local bench_post="${RESULT_DIR}/patched_submit_rep${rep}_post.yaml"

    build_submit_bench "$BENCH_SUBMIT_SRC" "$bench_pre" "pre" "$TPS" "$submit_pre_dir" "${submit_pre_dir}/warmup" "$MONITOR_INTERVAL_SEC"
    build_submit_bench "$BENCH_SUBMIT_SRC" "$bench_post" "post" "$TPS" "$submit_post_dir" "${submit_post_dir}/warmup" "$MONITOR_INTERVAL_SEC"

    if ! run_phase "$rep" "SUBMIT" "PREFAULT" "$bench_pre" "$NET_SUBMIT_TMP" "${SUBMIT_REAL_LABEL}-preFault" "$fault_nodes"; then
      should_continue_after_error || return 1
      log "WARNING: continuing after failed phase SUBMIT PREFAULT rep=${rep}"
    fi

    inject_faults_now "$rep" "SUBMIT" "POSTFAULT" "$K_FAULT"
    sleep "$POSTFAULT_GAP_SEC"

    if ! run_phase "$rep" "SUBMIT" "POSTFAULT" "$bench_post" "$NET_SUBMIT_TMP" "${SUBMIT_REAL_LABEL}-postFault" "$ACTUAL_FAULT_NODES_CSV"; then
      should_continue_after_error || return 1
      log "WARNING: continuing after failed phase SUBMIT POSTFAULT rep=${rep}"
    fi
  else
...
run_retrieve_experiment() {
  ...
  local fault_nodes
  fault_nodes="$(format_fault_nodes_csv "$rep" "$K_FAULT")"
  log "Retrieve fault set (resolved after network ready): ${fault_nodes}"
  ...
  if [ "$MODE" = "split" ]; then
    local bench_pre="${RESULT_DIR}/patched_retrieve_rep${rep}_pre.yaml"
    local bench_post="${RESULT_DIR}/patched_retrieve_rep${rep}_post.yaml"

    build_retrieve_bench "$BENCH_RETRIEVE_SRC" "$bench_pre" "pre" "$TPS" "$PRELOAD_RECEIPTS_DIR" "$MONITOR_INTERVAL_SEC"
    build_retrieve_bench "$BENCH_RETRIEVE_SRC" "$bench_post" "post" "$TPS" "$PRELOAD_RECEIPTS_DIR" "$MONITOR_INTERVAL_SEC"

    if ! run_phase "$rep" "RETRIEVE" "PREFAULT" "$bench_pre" "$NET_RETRIEVE_TMP" "${RETRIEVE_REAL_LABEL}-preFault" "$fault_nodes"; then
      should_continue_after_error || return 1
      log "WARNING: continuing after failed phase RETRIEVE PREFAULT rep=${rep}"
    fi

    inject_faults_now "$rep" "RETRIEVE" "POSTFAULT" "$K_FAULT"
    sleep "$POSTFAULT_GAP_SEC"

    if ! run_phase "$rep" "RETRIEVE" "POSTFAULT" "$bench_post" "$NET_RETRIEVE_TMP" "${RETRIEVE_REAL_LABEL}-postFault" "$ACTUAL_FAULT_NODES_CSV"; then
      should_continue_after_error || return 1
      log "WARNING: continuing after failed phase RETRIEVE POSTFAULT rep=${rep}"
    fi
  else
    local bench_unsplit="${RESULT_DIR}/patched_retrieve_rep${rep}_unsplit.yaml"
    build_retrieve_bench "$BENCH_RETRIEVE_SRC" "$bench_unsplit" "unsplit" "$TPS" "$PRELOAD_RECEIPTS_DIR" "$MONITOR_INTERVAL_SEC"
...
validate_fault_profile || exit 1

if [ "$K_FAULT" -ne 3 ]; then
  echo "ERROR: final fixed-placement experiment requires K_FAULT=3, but got ${K_FAULT}" >&2
  exit 1
fi
...
K_FAULT=${K_FAULT}
MODE=${MODE}
FAULT_PROFILE_RAW=${FAULT_PROFILE_RAW}
FAULT_PROFILE_NORMALIZED=${FAULT_PROFILE}
CC_NAME=${CC_NAME}
CC_VERSION=${CC_VERSION}
CC_SEQUENCE=${CC_SEQUENCE}
FAULT_CMD=${FAULT_CMD}
LIMITED_IMPACT_SET=${LIMITED_IMPACT_SET}
HIGH_IMPACT_SET=${HIGH_IMPACT_SET}
EXPECTED_ORDERERS=${EXPECTED_ORDERERS}
WARMUP_SEC=${WARMUP_SEC}
UNSPLIT_REAL_SEC=${UNSPLIT_REAL_SEC}
PREFAULT_SEC=${PREFAULT_SEC}
POSTFAULT_SEC=${POSTFAULT_SEC}
FAULT_AFTER_REAL_START_SEC=${FAULT_AFTER_REAL_START_SEC}
POSTFAULT_GAP_SEC=${POSTFAULT_GAP_SEC}
...
log "Fault profile raw       : ${FAULT_PROFILE_RAW}"
log "Fault profile normalized: ${FAULT_PROFILE}"
